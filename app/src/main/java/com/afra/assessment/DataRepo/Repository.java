...
public class Repository {
    ...
    public MutableLiveData<List<TransactionModel>> makeTransactionModel(TransactionModel transactionModel){
        final MutableLiveData<List<TransactionModel>> transactionList = new MutableLiveData<>();
        RetrofitInstance.apiBuilder().makeTransaction(transactionModel).enqueue(new Callback<List<TransactionModel>>() {
            @Override
            public void onResponse(Call<List<TransactionModel>> call, Response<List<TransactionModel>> response) {
                if(response.isSuccessful()&&response.body()!=null){
                    Log.d("response",response.body().toString());
